...
constexpr uint8_t _target_pin[] = TARGETS;
constexpr uint16_t _targets_value[] = TARGET_VALUE;
...
static_assert(COUNT(_target_pin)<=COUNT(_targets_value), "TARGET_VALUE doit être supérieur ou égal aux broches TARGETS définies");
